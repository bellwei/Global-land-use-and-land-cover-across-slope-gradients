...
# ================== 参数设置 ==================
# 读取参数
start_row <- 1     # 数据起始行号（对应Excel中第1行）
num_rows <- 6      # 需要读取的行数（包含标题行）
...
p <- ggplot(lucc_long, aes(x = Slope_Level, y = Index, group = `分组`)) +
  ...
  scale_color_manual(
    ...
    labels = c("Forest", "Shrubland", "Grassland", 
               "Cropland", "Artificial surface"),
  ...
  theme(
    ...
    legend.key.height = unit(1, "mm"),     # 图例项高度
...
# ================== 保存图片 ==================
ggsave(
  filename = "LUCC_Index.tiff",          # 输出文件名
  plot = p,                              # 绘图对象
  device = "tiff",                       # 文件格式
  dpi = 700,                            # 分辨率（印刷级）
  width = 9,                            # 宽度（厘米）
