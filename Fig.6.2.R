...
# tidyverse: 包含 dplyr（数据处理）、tidyr（数据整形）、ggplot2（可视化）等核心包
library(tidyverse)

# readxl: 专门用于读取 Excel 文件（.xlsx/.xls），不依赖 Java
library(readxl)

# ggplot2: 声明式图形语法，用于构建高质量科研图表
# （tidyverse 已包含，显式加载可提高代码可读性）
library(ggplot2)

# showtext: 让 R 的图形设备支持自定义字体（如 Times New Roman）。
...
# 注意：此代码读取的是 "LUCC坡度分布变化指数2.xlsx"（第二组数据）
...
# 前 6 个等级（1-6）对应第一时期（2000-2010），后 6 个（7-12）对应第二时期（2010-2020）。
# 每个等级代表一个坡度区间，单位为度（°）。
x_labels <- tibble(
  Slope_Level = 1:12,
...
plot_settings <- list(
  ...
  # 坐标轴线通用参数（Y轴，注意：此处的 axis 会覆盖 X轴的 axis 设置）
  # 由于 list 中键名重复，实际生效的是最后一个 axis 的定义
  axis = list(
    line_width = 0.1,   # 轴线粗细
...
# 前 6 个坡度等级（1-6）为第一时期（2000-2010），后 6 个（7-12）为第二时期（2010-2020）。
